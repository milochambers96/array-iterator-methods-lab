...
// ! Reminder, split creates substrings, which are then being put into an array. 

const firstLast = people.map((person) => {
    const personName = person.split(', ', 2);
    return personName[1] + " " + personName[0];
}) 
...
const isAdultPresent = devs.some((developer) => {
    ...
    return currentYear - developer.year >= 18;
})
...
const isEveryone19OrOlder = devs.every((developer) => {
    const currentYear = 2024;
    return currentYear - developer.year >= 19;
})
...
const travelMethodCounts = travelMethods.reduce((acc, transport) => {
    if (!acc.hasOwnProperty(transport)) {
        acc[transport] = 1 
    // If not listed, it adds the transport method to the new object with a count of 1.
    } else {
        acc[transport] += 1;
    } 
    // If the transport method is alreadyy listed, it increases the existing count by 1.
    return acc
}, {});

    /* Square brackets being used within the if/else statements as the keys names are not known, 
    they are being extracted from the array (travelethods) */
